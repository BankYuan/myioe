...
def cash_flow(request):
    ...
    # 快捷区间 —— 用本地时区的今天,避免容器系统时区是UTC导致日期错位
    from django.utils import timezone as _tz
    def _local_date(dt):
        """把 datetime/date 转成本地时区(Asia/Shanghai)的 date"""
        if dt is None:
            return None
        if hasattr(dt, 'date') and hasattr(dt, 'tzinfo'):
            return _tz.localtime(dt).date()
        if hasattr(dt, 'date'):
            return dt.date()
        return dt
    ...
    def _date_ok(d):
        if d is None:
            return False
        d = _local_date(d)   # 统一转本地日期,避免 UTC 凌晨错位
        if df and d < df:
            return False
        if dt and d > dt:
            return False
    ...
    if kind in ('', 'sale'):
        ...
        for s in sales:
            ...
            items.append({
                'when': d, 'biz_date': _local_date(d), 'kind': 'sale', 'kind_label': '销售',
                'desc': f'销售单 #{s.id} · {who}',
    ...
    if kind in ('', 'recharge'):
        for r in RechargeRecord.objects.select_related('member', 'operator'):
            ...
            if not _date_ok(d):
                continue
            items.append({
                'when': d, 'biz_date': _local_date(d), 'kind': 'recharge', 'kind_label': '充值',
    ...
    if kind in ('', 'purchase'):
        for t in InventoryTransaction.objects.filter(
            ...
            # 资金流水按"钱实际流动时间"(created_at 录入时间)过滤,不用 transaction_date(业务日期)
            # 否则今天录入的业务日期为历史的进货,在"查看今天"里看不到
            d = t.created_at
            if not _date_ok(d):
                continue
            ...
            items.append({
                'when': t.created_at, 'biz_date': _local_date(t.created_at),
